...
class Username extends Component {
  ...
  shouldComponentUpdate({ name, verified }) {
    const curProps = this.props;
    if (name !== curProps.name) return true;
    else if (verified !== curProps.verified) return true;
    return false;
  }

  render() {
    const { name, verified } = this.props;
    return (
...
const mSTP = ({ Authentication }) => {
  const { name, verified } = Authentication.user;
  return { name, verified };
}

export default connect(mSTP)(Username);